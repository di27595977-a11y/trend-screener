...
function detectTriangle(swingHighs: SwingPoint[], swingLows: SwingPoint[], totalBars: number) {
  ...
  if (endGap >= startGap) return null;
  ...
  if (highDown && lowUp) type = 'symmetric';
  else if (highFlat && lowUp) type = 'ascending';
  else if (highDown && lowFlat) type = 'descending';
  else if (highLine.slope < 0 && lowLine.slope < 0) type = 'fallingWedge';
  else if (highLine.slope > 0 && lowLine.slope > 0) type = 'risingWedge';
  else type = 'symmetric';
